-- Vertical 3F: Dashboard reads use the session-scoped effective gym authority.

create or replace function public.dashboard_actor_is_active()
returns boolean
language sql
stable
security definer
...
begin
  ...
  if v_gym_id is null or v_role not in ('admin','athlete','coach') then
    return;
  end if;
  return query
  select g.id, g.name, g.logo_url, v_role,
    ...
    public.is_registered_web_session()
  from public.gyms g
...
begin
  ...
  if v_gym_id is null or public.effective_gym_role() <> 'admin'
    or not public.dashboard_actor_is_active() then
...
begin
  ...
  if auth.uid() is null then raise exception using errcode='P0001',message='unauthenticated'; end if;
  if v_gym_id is null or public.effective_gym_role()<>'admin'
    or not public.dashboard_actor_is_active() then raise exception using errcode='42501',message='forbidden'; end if;
...
$function$;
revoke all on function public.dashboard_actor_is_active() from public,anon,authenticated,service_role;
revoke all on function public.get_effective_dashboard_context() from public,anon;
revoke all on function public.list_effective_dashboard_members() from public,anon;
revoke all on function public.get_effective_pending_join_request_count() from public,anon;
revoke all on function public.list_gym_activity(integer,integer) from public,anon;
revoke all on function public.list_members_without_recent_bookings(integer,integer,integer) from public,anon;
revoke all on function public.get_coach_class_summary(text) from public,anon;
grant execute on function public.get_effective_dashboard_context() to authenticated,service_role;
grant execute on function public.list_effective_dashboard_members() to authenticated,service_role;
grant execute on function public.get_effective_pending_join_request_count() to authenticated,service_role;
grant execute on function public.list_gym_activity(integer,integer) to authenticated,service_role;
grant execute on function public.list_members_without_recent_bookings(integer,integer,integer) to authenticated,service_role;
grant execute on function public.get_coach_class_summary(text) to authenticated,service_role;
