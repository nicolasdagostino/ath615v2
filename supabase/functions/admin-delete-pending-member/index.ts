...
serve(async (req) => {
  ...
  try {
    ...
    if (adminProfile.role !== 'admin' && adminProfile.role !== 'owner') {
      throw new Error('Only admins can delete pending members')
    }
    ...
    const { data: memberUser, error: getUserError } =
      await adminClient.auth.admin.getUserById(memberId)

    if (getUserError || !memberUser.user) {
      throw new Error('Member auth user not found')
    }

    if (memberUser.user.email_confirmed_at || memberUser.user.last_sign_in_at) {
      throw new Error('Only pending members can be deleted')
    }
    ...
    for (const [table, column] of deletes) {
      const { error } = await adminClient.from(table).delete().eq(column, memberId)
    ...
    if (deleteAuthError) {
      throw new Error(`Could not delete auth user: ${deleteAuthError.message}`)
    }
